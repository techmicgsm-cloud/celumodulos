...
export async function actualizarImportacion(
  ...
  if (!user) {
    return { error: "Debes iniciar sesión para editar una importación." };
  }
  ...
  if (!totalUsd || totalUsd <= 0) return { error: "El total de mercadería en USD debe ser mayor a 0." };
  if (!gastoPesos || gastoPesos <= 0) return { error: "El gasto total en pesos debe ser mayor a 0." };

  let items: (ItemRecibido & { id?: string })[] = [];
  try {
    items = JSON.parse(itemsRaw ?? "[]");
  } catch {
    return { error: "Los modelos enviados tienen un formato inválido." };
  ...
  if (items.length === 0) return { error: "Agregá al menos un modelo antes de guardar." };
  for (const item of items) {
    if (!item.modelo?.trim()) return { error: "Todos los modelos necesitan un nombre." };
    if (!item.cantidad || item.cantidad <= 0) return { error: `La cantidad de "${item.modelo}" debe ser mayor a 0.` };
    if (!item.precioUsdUnitario || item.precioUsdUnitario <= 0) return { error: `El precio USD de "${item.modelo}" debe ser mayor a 0.` };
  }

  const factor = calcularFactor(gastoPesos, totalUsd);

  // Obtener items actuales para comparar stock vendido
  const { data: currentItems, error: errorCurrent } = await supabase
    .from("importacion_items")
    .select("id, cantidad_inicial, cantidad_disponible, modelo")
    .eq("importacion_id", importacionId);

  if (errorCurrent) return { error: "Error al verificar el stock actual de la importación." };

  // Validaciones de stock
  const currentMap = new Map((currentItems || []).map(i => [i.id, i]));
  const incomingIds = new Set(items.map(i => i.id).filter(Boolean));

  for (const curr of currentItems || []) {
    const cantidadVendida = curr.cantidad_inicial - curr.cantidad_disponible;
    // Si lo borró
    if (!incomingIds.has(curr.id) && cantidadVendida > 0) {
      return { error: `No puedes eliminar el modelo "${curr.modelo}" porque ya se vendieron ${cantidadVendida} unidades.` };
    }
  }
  ...
  for (const item of items) {
    if (item.id && currentMap.has(item.id)) {
      ...
      const cantidadVendida = curr.cantidad_inicial - curr.cantidad_disponible;
      const nuevaDisponible = item.cantidad - cantidadVendida;
      if (nuevaDisponible < 0) {
        return { error: `No puedes bajar la cantidad de "${item.modelo}" a ${item.cantidad}. Ya se vendieron ${cantidadVendida} unidades.` };
      }
    }
  }

  // Upload factura if any
  let facturaUpdates: any = {};
  if (factura && factura.size > 0) {
    const extension = factura.name.split(".").pop() || "bin";
    const path = `${user.id}/${Date.now()}-${randomUUID()}.${extension}`;
    const arrayBuffer = await factura.arrayBuffer();
    ...
    const { error: errorSubida } = await supabase.storage
      .from(BUCKET_FACTURAS)
      .upload(path, arrayBuffer, { contentType: factura.type || "application/octet-stream" });

    if (errorSubida) return { error: `No se pudo subir la factura: ${errorSubida.message}` };
    facturaUpdates = { factura_path: path, factura_nombre_archivo: factura.name };
  }
  ...
  if (errorUpdate) return { error: "No se pudo actualizar la importación." };

  // Process items
  for (const curr of currentItems || []) {
    if (!incomingIds.has(curr.id)) {
      await supabase.from("importacion_items").delete().eq("id", curr.id);
    }
  }
  ...
  for (const item of items) {
    ...
    if (item.id && currentMap.has(item.id)) {
      ...
      const cantidadVendida = curr.cantidad_inicial - curr.cantidad_disponible;
      const nuevaDisponible = item.cantidad - cantidadVendida;
      await supabase.from("importacion_items").update({
        marca: item.marca?.trim() || null,
        categoria: item.categoria?.trim() || null,
        modelo: item.modelo.trim(),
        sku: item.sku?.trim() || null,
